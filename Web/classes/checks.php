...
class checks
{
	...
	function trashmail($email) {
		...
		$trashmaillist = file_get_contents('includes/throwaway_domains.txt');
		if(strcasecmp($trashmaillist, $emaildomain) == 0) {
			return true;
		}
		return false;
	}
	...
	function PrivIP($ip){
		...
		if ($ip <= ip2long('0.255.255.255') && ip2long('0.0.0.0') <= $ip) {
			return 'Current network (only valid as source address).';
		}elseif ($ip <= ip2long('10.255.255.255') && ip2long('10.0.0.0') <= $ip) {
			return 'Used for local communications within a private network.';
		}elseif ($ip <= ip2long('100.127.255.255') && ip2long('100.64.0.0') <= $ip) {
			return 'Shared address space[6] for communications between a service provider and its subscribers when using a carrier-grade NAT.';
		}elseif ($ip <= ip2long('127.255.255.255') && ip2long('127.0.0.0') <= $ip) {
			return 'Used for loopback addresses to the local host.';
		}elseif ($ip <= ip2long('169.254.255.255') && ip2long('169.254.0.0') <= $ip) {
			return 'Used for link-local addresses[7] between two hosts on a single link when no IP address is otherwise specified, such as would have normally been retrieved from a DHCP server.';
		}elseif ($ip <= ip2long('172.31.255.255') && ip2long('172.16.0.0') <= $ip) {
			return 'Used for local communications within a private network.';
		}elseif ($ip <= ip2long('192.0.0.255') && ip2long('192.0.0.0') <= $ip) {
			return 'ETF Protocol Assignments.';
		}elseif ($ip <= ip2long('192.0.2.255') && ip2long('192.0.2.0') <= $ip) {
			return 'Assigned as TEST-NET-1, documentation and examples.';
		}elseif ($ip <= ip2long('192.168.255.255') && ip2long('192.168.0.0') <= $ip) {
			return 'Used for local communications within a private network.';
		}elseif ($ip <= ip2long('198.19.255.255') && ip2long('198.18.0.0') <= $ip) {
			return 'Used for benchmark testing of inter-network communications between two separate subnets.';
		}elseif ($ip <= ip2long('198.51.100.255') && ip2long('198.51.100.0') <= $ip) {
			return 'Assigned as TEST-NET-2, documentation and examples.';
		}elseif ($ip <= ip2long('203.0.113.255') && ip2long('203.0.113.0') <= $ip) {
			return 'Assigned as TEST-NET-3, documentation and examples.';
		}elseif ($ip <= ip2long('239.255.255.255') && ip2long('224.0.0.0') <= $ip) {
			return 'In use for IP multicast';
		}elseif ($ip <= ip2long('255.255.255.254') && ip2long('240.0.0.0') <= $ip) {
			return 'Reserved for future use.';
		}elseif ($ip <= ip2long('255.255.255.255') && ip2long('255.255.255.255') <= $ip) {
			return 'Reserved for the "limited broadcast" destination address.';
		}
		return 'OK';
	}
	...
	function LookupIP($ip) {
		...
		$sql = "SELECT str FROM whois WHERE (INET_ATON(:ip) BETWEEN INET_ATON(ipf) AND INET_ATON(ipt)) AND dDate > NOW() - INTERVAL 2 WEEK ORDER BY dDate DESC LIMIT 1";
        ...
        if ($query->rowCount() > 0) {
			...
			$ipf =  trim(explode(" - ", $myObj['inetnum'])[0]);
			$ipt = trim(explode(" - ", $myObj['inetnum'])[1]);
			
			if(!$ipf){
				$ipf =  trim(explode(" - ", $myObj['NetRange'])[0]);
				$ipt = trim(explode(" - ", $myObj['NetRange'])[1]);
			}
			
			$descr = trim($myObj['descr']);
			if(!$descr){
				$descr = trim($myObj['OrgName']);
			}
			
			$country = trim($myObj['country']);
			if(!$country){
				$country = trim($myObj['Country']);
			}
			
			$origin = str_replace("AS", "", $myObj['origin']);
			$origin = trim($origin);
			$source = trim($myObj['source']);
			
			$sql_ins = "insert into whois (ipf,ipt,country,descr,asn,str) values (:ipf,:ipt,:country,:descr,:asn,:str)";
            $query_ins = $this->dbh->prepare($sql_ins);
            $query_ins->bindParam(':ipf', $ipf, PDO::PARAM_STR);
            $query_ins->bindParam(':ipt', $ipt, PDO::PARAM_STR);
            $query_ins->bindParam(':country', $country, PDO::PARAM_STR);
            $query_ins->bindParam(':descr', $descr, PDO::PARAM_STR);
            $query_ins->bindParam(':asn', $origin, PDO::PARAM_STR);
            $query_ins->bindParam(':str', $result, PDO::PARAM_STR);
